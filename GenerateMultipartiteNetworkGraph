...
def draw_graph(graph, labels=None, graph_layout='reingold',
               ...
               node_text_size=5,
               edge_color='blue', edge_alpha=0.1, edge_tickness=1,
               edge_text_pos=0.3,
    ...
    nx.draw_networkx_nodes(G, graph_pos, nodelist=techlist, node_size=300,
                           alpha=node_alpha, node_color='orange')
...
for i in range(1, len(TechToEffort_py)-1): #number of rows in the matrix (ie, numbers of technology)
    for j in range(1, 6): #iterates over number of efforts
        if TechToEffort_py[i][j] == 1: #checks if a link exists, ie if a '1' is in the matrix
...
#biaslist = biaslist.sort() #for some reason this doesn't work. not necessary
...
column_count = 0
while bcount < (biases_per_column*2 -2):
    bcount = bcount + 1
    biasPosition.append((-0.5,-0.9 + (scatterBias*column_count)))
    column_count = column_count + 1

column_count = 0
while bcount < (biases_per_column*3 -3):
    bcount = bcount + 1
...
while bcount < (biases_per_column*4):
    bcount = bcount + 1
    biasPosition.append((0,-0.9 + (scatterBias*column_count)))
    column_count = column_count + 1
